...
const { default: H1 } = require("shared/components/H1");
const { default: styled } = require("styled-components");

export default function Habito({ habito, track, setFinal }) {
  return (
    <HabitoWrapper>
      <DescriptionWrapper data-identifier="today-infos">
        <H1 cor="#666">{habito.name}</H1>
        <Text>
          Sequencia atual:
          <Marcador active={habito.done}>
        ...
        <Text>
          ...
          <Marcador
            active={
              habito.highestSequence >= habito.currentSequence && habito.done
            }
          >
      ...
      <Buttonsuccess
        ...
        onClick={() => {
          const searchHabito = track.habitos.find((x) => x.id === habito.id);
          searchHabito.done = !searchHabito.done;
          track.setHabitos([...track.habitos]);
          Api.post(
            `habits/${habito.id}/${searchHabito.done ? "check" : "uncheck"}`
          ).then(() => setFinal(true));
        }}
      >
...
const Marcador = styled.span`
  color: ${({ active }) => (active ? "#8FC549" : "#666666")};
`;
